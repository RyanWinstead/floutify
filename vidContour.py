...
class vidContour():
    def __init__(self, fileName):
        ...
        cv2.createTrackbar( 'start', 'cellVid', 0, self.length, self.onChange)
    ...
    def onChange(self, trackbarValue):
        ...
        cnts, image_edged3, image_contours = imagePros(current_frame, 'vid')
	   # print(cnts, image_edged3, image_contours)
        ...
        if k == 27:
            ...
            print(len(bboxes))
            print(cellnumber)
            bbox_coords = [bboxes[cellnumber][0], bboxes[cellnumber][1], bboxes[cellnumber][2], bboxes[cellnumber][3]]
            tracking.track(bbox_coords, cv2.CAP_PROP_POS_FRAMES, self.fileName)
