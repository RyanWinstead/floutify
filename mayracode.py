...
counter = 0;
image1 = cv2.imread("cellimage1.jpg");
image = cv2.imread("cellimage1.jpg", 0);
...
image_norm= cv2.normalize(cl1, dst, 0, 255, cv2.NORM_MINMAX)
#Gray Scale GaussianBlur
image_gray = cv2.GaussianBlur(image_norm, (3,3), 0,0);
...
#Find Contours
cnts = cv2.findContours(image_gray.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE);
...
counter = counter;
#Return Results
print ("There are "+ str(counter) + " cells.");
